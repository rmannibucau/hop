...
public class KubernetesDeleteDialog extends BaseTransformDialog {

  private static final Class<?> PKG = KubernetesApiService.class;

  protected final KubernetesDeleteMeta input;
  ...
  protected TextVar wNameFieldName;
  protected TextVar wNamespace;
  protected TextVar wLabelSelector;
  protected TextVar wOutputFieldName;
  protected Button wIgnoreNotFound;

  public KubernetesDeleteDialog(
      final Shell parent,
      final IVariables variables,
      final KubernetesDeleteMeta transformMeta,
  ...
  @Override
  public String open() {
    createShell(BaseMessages.getString(PKG, "KubernetesDeleteDialog.Shell.Title"));
    ...
    wConnection =
        new MetaSelectionLine<>(
            ...
            wComposite,
            SWT.NONE,
            BaseMessages.getString(PKG, "KubernetesDeleteDialog.Connection.Label"),
            BaseMessages.getString(PKG, "KubernetesDeleteDialog.Connection.Tooltip"));
    ...
    wSettings.setText(BaseMessages.getString(PKG, "KubernetesDeleteDialog.Settings.Label"));
    ...
    wKind =
        addVarField(
            wSettings,
            "KubernetesDeleteDialog.Kind.Label",
            "KubernetesDeleteDialog.Kind.Tooltip",
            null);
    wApiVersion =
        addVarField(
            wSettings,
            "KubernetesDeleteDialog.ApiVersion.Label",
            "KubernetesDeleteDialog.ApiVersion.Tooltip",
            wKind);
    wName =
        addVarField(
            wSettings,
            "KubernetesDeleteDialog.Name.Label",
            "KubernetesDeleteDialog.Name.Tooltip",
            wApiVersion);
    wNameFieldName =
        addVarField(
            wSettings,
            "KubernetesDeleteDialog.NameFieldName.Label",
            "KubernetesDeleteDialog.NameFieldName.Tooltip",
            wName);
    wNamespace =
        addVarField(
            wSettings,
            "KubernetesDeleteDialog.Namespace.Label",
            "KubernetesDeleteDialog.Namespace.Tooltip",
            wNameFieldName);
    wLabelSelector =
        addVarField(
            wSettings,
            "KubernetesDeleteDialog.LabelSelector.Label",
            "KubernetesDeleteDialog.LabelSelector.Tooltip",
            wNamespace);
    wOutputFieldName =
        addVarField(
            wSettings,
            "KubernetesDeleteDialog.OutputFieldName.Label",
            "KubernetesDeleteDialog.OutputFieldName.Tooltip",
            wLabelSelector);

    wIgnoreNotFound = new Button(wSettings, SWT.CHECK | SWT.LEFT);
    wIgnoreNotFound.setText(
        BaseMessages.getString(PKG, "KubernetesDeleteDialog.IgnoreNotFound.Label"));
    wIgnoreNotFound.setToolTipText(
        BaseMessages.getString(PKG, "KubernetesDeleteDialog.IgnoreNotFound.Tooltip"));
    PropsUi.setLook(wIgnoreNotFound);
    final FormData fdIgnoreNotFound = new FormData();
    fdIgnoreNotFound.left = new FormAttachment(0, 0);
    fdIgnoreNotFound.top = new FormAttachment(wOutputFieldName, margin);
    wIgnoreNotFound.setLayoutData(fdIgnoreNotFound);
    wIgnoreNotFound.addSelectionListener(
  ...
  protected void getData() {
    ...
    wOutputFieldName.setText(Const.NVL(input.getOutputFieldName(), "deleted"));
    wIgnoreNotFound.setSelection(input.isIgnoreNotFound());
  }

  protected void ok() {
    if (Utils.isEmpty(wConnection.getText())) {
      final MessageBox mb = new MessageBox(shell, SWT.ICON_ERROR | SWT.OK);
      mb.setMessage(BaseMessages.getString(PKG, "KubernetesDeleteDialog.Error.NoConnection"));
      mb.open();
      return;
    }
    getInfo(input);
  ...
  protected void getInfo(final KubernetesDeleteMeta in) {
    ...
    in.setNameFieldName(wNameFieldName.getText());
    ...
    in.setIgnoreNotFound(wIgnoreNotFound.getSelection());
    in.setChanged();
  }
