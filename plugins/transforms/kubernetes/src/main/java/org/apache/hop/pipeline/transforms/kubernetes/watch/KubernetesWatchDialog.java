...
public class KubernetesWatchDialog extends BaseTransformDialog {

  private static final Class<?> PKG = KubernetesApiService.class;

  protected final KubernetesWatchMeta input;
  ...
  protected Button wIncludeBody;

  public KubernetesWatchDialog(
      final Shell parent,
      final IVariables variables,
      final KubernetesWatchMeta transformMeta,
  ...
  @Override
  public String open() {
    createShell(BaseMessages.getString(PKG, "KubernetesWatchDialog.Shell.Title"));
    ...
    wConnection =
        new MetaSelectionLine<>(
            ...
            wComposite,
            SWT.NONE,
            BaseMessages.getString(PKG, "KubernetesWatchDialog.Connection.Label"),
            BaseMessages.getString(PKG, "KubernetesWatchDialog.Connection.Tooltip"));
    ...
    wSettings.setText(BaseMessages.getString(PKG, "KubernetesWatchDialog.Settings.Label"));
    ...
    wKind =
        addVarField(
            wSettings,
            "KubernetesWatchDialog.Kind.Label",
            "KubernetesWatchDialog.Kind.Tooltip",
            null);
    wApiVersion =
        addVarField(
            wSettings,
            "KubernetesWatchDialog.ApiVersion.Label",
            "KubernetesWatchDialog.ApiVersion.Tooltip",
            wKind);
    wNamespace =
        addVarField(
            wSettings,
            "KubernetesWatchDialog.Namespace.Label",
            "KubernetesWatchDialog.Namespace.Tooltip",
            wApiVersion);
    wLabelSelector =
        addVarField(
            wSettings,
            "KubernetesWatchDialog.LabelSelector.Label",
            "KubernetesWatchDialog.LabelSelector.Tooltip",
            wNamespace);
    wOutputFieldName =
        addVarField(
            wSettings,
            "KubernetesWatchDialog.OutputFieldName.Label",
            "KubernetesWatchDialog.OutputFieldName.Tooltip",
            wLabelSelector);

    wIncludeBody = new Button(wSettings, SWT.CHECK | SWT.LEFT);
    wIncludeBody.setText(BaseMessages.getString(PKG, "KubernetesWatchDialog.IncludeBody.Label"));
    wIncludeBody.setToolTipText(
        BaseMessages.getString(PKG, "KubernetesWatchDialog.IncludeBody.Tooltip"));
    PropsUi.setLook(wIncludeBody);
    final FormData fdIncludeBody = new FormData();
    fdIncludeBody.left = new FormAttachment(0, 0);
    fdIncludeBody.top = new FormAttachment(wOutputFieldName, margin);
    wIncludeBody.setLayoutData(fdIncludeBody);
    wIncludeBody.addSelectionListener(
  ...
  protected void getData() {
    ...
    wOutputFieldName.setText(Const.NVL(input.getOutputFieldName(), "watch_event"));
    wIncludeBody.setSelection(input.isIncludeBody());
  }

  protected void ok() {
    if (Utils.isEmpty(wConnection.getText())) {
      final MessageBox mb = new MessageBox(shell, SWT.ICON_ERROR | SWT.OK);
      mb.setMessage(BaseMessages.getString(PKG, "KubernetesWatchDialog.Error.NoConnection"));
      mb.open();
      return;
    }
    getInfo(input);
  ...
  protected void getInfo(final KubernetesWatchMeta in) {
    ...
    in.setIncludeBody(wIncludeBody.getSelection());
    in.setChanged();
  }
