...
/** Dialog for the Kubernetes Scale action. */
public class ActionKubernetesScaleDialog extends ActionDialog {

  private static final Class<?> PKG = ActionKubernetesScaleDialog.class;

  private ActionKubernetesScale action;
  ...
  private Text wReplicas;
  private Button wWaitReady;
  private Text wTimeoutSeconds;
  private boolean changed;

  public ActionKubernetesScaleDialog(
      Shell parent, ActionKubernetesScale action, WorkflowMeta workflowMeta, IVariables variables) {
    super(parent, workflowMeta, variables);
    this.action = action;
    if (action.getName() == null) {
      action.setName(BaseMessages.getString(PKG, "ActionKubernetesScale.Name"));
    }
  }

  @Override
  public IAction open() {
    createShell(BaseMessages.getString(PKG, "ActionKubernetesScale.Dialog.Title"), action);
  ...
  private void createGeneralTab(Composite wGeneralComp, int margin) {
    wConnection =
        new MetaSelectionLine<>(
            ...
            wGeneralComp,
            SWT.NONE,
            BaseMessages.getString(PKG, "ActionKubernetesScale.Dialog.Connection.Label"),
            null);
    ...
    last = addLabel(wGeneralComp, "ActionKubernetesScale.Dialog.Kind.Label", last, margin);
    wKind = addTextVar(wGeneralComp, last, margin);
    last = wKind;

    last = addLabel(wGeneralComp, "ActionKubernetesScale.Dialog.Name.Label", last, margin);
    wName = addTextVar(wGeneralComp, last, margin);
    last = wName;

    last = addLabel(wGeneralComp, "ActionKubernetesScale.Dialog.Namespace.Label", last, margin);
    wNamespace = addTextVar(wGeneralComp, last, margin);
    last = wNamespace;

    last = addLabel(wGeneralComp, "ActionKubernetesScale.Dialog.Replicas.Label", last, margin);
    wReplicas = addInt(wGeneralComp, last, margin);
    last = wReplicas;

    wWaitReady =
        addCheck(wGeneralComp, "ActionKubernetesScale.Dialog.WaitReady.Label", last, margin);
    last = wWaitReady;

    last =
        addLabel(wGeneralComp, "ActionKubernetesScale.Dialog.TimeoutSeconds.Label", last, margin);
  ...
  public void getData() {
    ...
    wReplicas.setText(action.getReplicas() > 0 ? Integer.toString(action.getReplicas()) : "");
  ...
  private void ok() {
    ...
    String replicas = wReplicas.getText();
    action.setReplicas(Utils.isEmpty(replicas) ? 0 : Integer.parseInt(replicas.trim()));
